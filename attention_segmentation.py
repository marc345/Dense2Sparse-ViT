...
def save_image_grid(num_rows, num_cols, images, args, epoch_num, patch_scores=None, head=None,
    ...
    for row in range(num_rows):
        for col in range(num_cols):
            ...
            axs[head, layer].set_title(title_str, fontsize=10)
            axs[head, layer].imshow(images[(row * num_cols) + col], interpolation='nearest')
            if patch_scores is not None:
                current_scores = np.round(patch_scores[(row * num_cols) + col], 4)
                heatmap = axs[head, layer].imshow(current_scores, cmap='cool', alpha=0.35, vmin=0, vmax=1)
