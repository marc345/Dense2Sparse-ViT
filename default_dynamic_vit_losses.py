...
class DiffPruningLoss(torch.nn.Module):
    ...
    def __init__(self, base_criterion: torch.nn.Module, dynamic=False, ratio_weight=2.0, pruning_loc=[3, 6, 9],
                 keep_ratio=[0.75, 0.5, 0.25], clf_weight=0, print_mode=True):
        super().__init__()
        self.base_criterion = base_criterion
        ...
        if self.print_mode:
            ...
            if self.count % 100 == 0:
                print('loss info: cls_loss=%.4f, ratio_loss=%.4f, cls_kl=%.4f, token_kl=%.4f\n'
                      'cls_weight=%.2f, ratio_weight=%.2f, distill_weight=%.2f' % (
                self.cls_loss / self.count, self.ratio_loss / self.count, self.cls_distill_loss / self.count,
                self.token_distill_loss / self.count, self.clf_weight, self.ratio_weight, self.distill_weight))
        return loss
