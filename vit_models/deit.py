...
class DistilledVisionTransformer(VisionTransformer):
    ...
    def forward_selfattention(self, x):
        ...
        # distillation token and CLS token
        if self.pos_embed.shape[1] == 198:
            N = self.pos_embed.shape[1] - 2
            dist_pos_embed = self.pos_embed[:, 1]
            patch_pos_embed = self.pos_embed[:, 2:]
...
class MaskedDistilledVisionTransformer(DistilledVisionTransformer):
    def __init__(self, *args, **kwargs):
        super(MaskedDistilledVisionTransformer, self).__init__(*args, **kwargs)
    ...
    def forward_features(self, x, masks):
        ...
        dist_keep_decision = torch.ones((B, 1, 1), dtype=masks.dtype, device=masks.device)
        keep_decisions = torch.cat((cls_keep_decision, dist_keep_decision, patch_keep_decisions), dim=1)
        ...
        layer_wise_tokens = [self.norm(x) for x in layer_wise_tokens]
        return [(x[:, 0], x[:, 1]) for x in layer_wise_tokens], keep_decisions
    ...
    def forward(self, x, masks):
        ...
        x_dist = [self.head_dist(x_dist) for _, x_dist in list_out]
        #if self.training:
        return [(out, out_dist) for out, out_dist in zip(x, x_dist)], keep_decisions
        #else:
        #    # during inference, return the average of both classifier predictions
        #    return [(out + out_dist) / 2 for out, out_dist in zip(x, x_dist)], keep_decisions

class MaskPredictorVisionTransformer(VisionTransformer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.patch_predictor = nn.Sequential(torch.nn.Linear(self.embed_dim, self.embed_dim*2),
                                             torch.nn.Linear(self.embed_dim*2, 2),
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if i == len(self.blocks) - 2:
                patch_class_probabilities = self.patch_predictor(x[:, 2:])
                patch_keep_decisions = F.gumbel_softmax(patch_class_probabilities, hard=True)[:, :, 0:1]
                self.keeping_ratio = torch.sum(patch_keep_decisions[:, :, 0].detach()) / (
                        patch_keep_decisions.shape[0] * patch_keep_decisions.shape[1])

                cls_keep_decision = torch.ones((B, 1, 1), dtype=x.dtype, device=x.device)
                dist_keep_decision = torch.ones((B, 1, 1), dtype=x.dtype, device=x.device)
                keep_decisions = torch.cat((cls_keep_decision, dist_keep_decision, patch_keep_decisions), dim=1)
    ...
    def forward(self, x):
        list_out, keep_decisions = self.forward_features(x)
        x = [self.head(x) for x, _ in list_out]
        x_dist = [self.head_dist(x_dist) for _, x_dist in list_out]
        #if self.training:
        return [(out, out_dist) for out, out_dist in zip(x, x_dist)], keep_decisions
...
@register_model
def deit_small_patch16_224_masked(pretrained=False, **kwargs):
    model = MaskedDistilledVisionTransformer(
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
...
@register_model
def deit_small_patch16_224_predictor(pretrained=False, **kwargs):
    model = MaskPredictorVisionTransformer(
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
